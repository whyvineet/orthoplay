...
const SpellingGame = ({
  ...
  const handlePaste = (e) => {
    ...
    if (chars.length) {
      ...
      setGuess(newGuess);
      const nextIndex =
        chars.length < wordLength ? chars.length : wordLength - 1;
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {Array.from({ length: wordLength }).map((_, index) => (
              <input
                ...
                maxLength={1}
                disabled={isLoading}
                className={`w-10 h-10 md:w-14 md:h-14 border-2 ${
                  darkMode
                    ? "border-gray-600 text-gray-200 bg-gray-700"
                    : " border-gray-300 text-gray-900 bg-gray-50"
                }  rounded-lg flex items-center justify-center text-2xl font-bold   focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-200 text-center uppercase transition-all duration-200 hover:border-blue-400 hover:shadow-md hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:shadow-none`}
              />
            ))}
          </div>
        </div>
